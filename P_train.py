...
from train.subsample import MaskFunc, UniformMaskFunc, RandomMaskFunc, PMaskFunc
from data.input_transforms import Prefetch2Device,TrainPreProcessCroppedK, TrainPreProcessPCK
from data.output_transforms import OutputReplaceTransformCroppedK

from models.P_unet import PCUnet
from train.model_trainers.model_trainer_PC import ModelTrainerIMG
...
def train_img(args):
    ...
    mask_func = PMaskFunc(args.center_fractions, args.accelerations)

    data_prefetch = Prefetch2Device(device)

    input_train_transform = TrainPreProcessPCK(mask_func, args.challenge, args.device,
                                                     use_seed=False, divisor=divisor)
    input_val_transform = TrainPreProcessPCK(mask_func, args.challenge, args.device,
    ...
    model = PCUnet(in_chans=data_chans, out_chans=data_chans, chans=args.chans,
