...
def _ssim(input, target, max_val, k1, k2, kernel):
    c1 = (k1 * max_val) ** 2
    c2 = (k2 * max_val) ** 2
...
class SSIMLoss(_Loss):
    ...
    def __init__(self, filter_size=11, k1=0.01, k2=0.03, sigma=1.5, size_average=None, reduce=None, reduction='mean'):
        ...
        self.k2 = k2
        self.sigma = sigma
        self.kernel = _fspecial_gaussian(filter_size, sigma)
...
class MultiScaleSSIMLoss(_Loss):
    ...
    def forward(self, input, target, weights=[0.0448, 0.2856, 0.3001, 0.2363, 0.1333], max_val=1.):
        return 1 - ms_ssim_loss(input, target, max_val=max_val, k1=self.k1, k2=self.k2, sigma=self.sigma,
                                kernel=self.kernel, weights=weights, filter_size=self.filter_size,
