...
from data.input_transforms import Prefetch2Device, TrainPreProcessK, TrainPreProcessCroppedK
from data.output_transforms import OutputReplaceTransformK, OutputReplaceTransformCroppedK

# from models.ks_unet import UnetKS
from models.fully_connected_unet_model import UnetModel
from train.model_trainers.model_trainer_IMG_vanilla import ModelTrainerIMG
...
def train_img(args):
    ...
    input_train_transform = TrainPreProcessCroppedK(mask_func, args.challenge, args.device, use_seed=False, divisor=divisor)
    input_val_transform = TrainPreProcessCroppedK(mask_func, args.challenge, args.device, use_seed=True, divisor=divisor)
    ...
    losses = dict(
        ...
        semi_kspace_loss=nn.MSELoss(reduction='mean'),
    ...
    model = UnetModel(in_chans=data_chans, out_chans=data_chans, chans=args.chans, num_pool_layers=args.num_pool_layers).to(device)
...
if __name__ == '__main__':
    settings = dict(
        ...
        lr_red_epoch=15,
        lr_red_rate=0.1,
        # prev_model_ckpt='checkpoints/IMG/Trial 07  2019-07-17 11-24-00/ckpt_032.tar',
    )
