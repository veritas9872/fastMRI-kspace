...
class ConvDownBlock(nn.Module):
    def __init__(self, in_chans, out_chans):
        ...
        self.layers = nn.Sequential(
            nn.Conv2d(in_chans, out_chans, kernel_size=3, padding=1, bias=False, stride=2),
            nn.BatchNorm2d(num_features=out_chans),
            nn.ReLU()
        )
...
class UnetKS(nn.Module):
    def __init__(self, in_chans, out_chans, ext_chans, chans, num_pool_layers,
        ...
        for n in range(num_pool_layers - 1):
            ...
            ch *= 2
            self.pool_layers.append(ConvDownBlock(in_chans=ch, out_chans=ch))
    ...
    def forward(self, tensor):
        ...
        for layer, pool in zip(self.down_sample_layers, self.pool_layers):
            output = layer(output)
            stack.append(output)
            output = pool(output)
