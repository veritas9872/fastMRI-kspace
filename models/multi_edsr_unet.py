...
class UNet(nn.Module):
    def __init__(self, in_chans, out_chans, chans, num_pool_layers,
                 num_res_layers, num_depth_blocks, res_scale=0.1, reduction=16, thick_base=False):
        super().__init__()
        ...
        self.down_res_blocks = nn.ModuleList([block])
        ...
        assert len(self.down_reshape_layers) == len(self.down_res_blocks) == len(self.upscale_layers) \
            == len(self.up_reshape_layers) == self.num_pool_layers, 'Layer number error!'
