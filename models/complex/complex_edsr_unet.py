...
class ComplexEDSRUNet(nn.Module):
    def __init__(self, in_chans: int, out_chans: int, chans: int, num_pool_layers: int,
                 num_depth_blocks: int, res_scale=0.1):
        super().__init__()
        self.num_pool_layers = num_pool_layers
        conv = ComplexConvLayer(in_chans=in_chans, out_chans=chans, stride=1)
        res = ComplexResBlock(num_chans=chans, res_scale=res_scale)
        self.down_reshape_layers = nn.ModuleList([conv])
        self.down_res_blocks = nn.ModuleList([res])

        ch = chans
        for _ in range(num_pool_layers - 1):
            conv = ComplexConvLayer(in_chans=ch, out_chans=ch * 2, stride=2)
            res = ComplexResBlock(num_chans=ch * 2, res_scale=res_scale)
            self.down_reshape_layers.append(conv)
            self.down_res_blocks.append(res)
            ch *= 2

        self.mid_conv = ComplexConvLayer(in_chans=ch, out_chans=ch, stride=2)

        mid_res_blocks = list()
        for _ in range(num_depth_blocks):
            res = ComplexResBlock(num_chans=ch, res_scale=res_scale)
        ...
        for _ in range(num_pool_layers - 1):
            ...
            conv = ComplexConvLayer(in_chans=ch * 2, out_chans=ch // 2, stride=1)
            res = ComplexResBlock(num_chans=ch // 2, res_scale=res_scale)
            ...
            deconv = ComplexResizeConv(in_chans=ch, out_chans=ch, scale_factor=2)
            conv = ComplexConvLayer(in_chans=ch * 2, out_chans=ch, stride=1)
            res = ComplexResBlock(num_chans=ch, res_scale=res_scale)
            self.upscale_layers.append(deconv)
            self.up_reshape_layers.append(conv)
            self.up_res_blocks.append(res)
