...
from data.mri_data import CustomSliceData, CustomSliceDataEnd
...
def create_prefetch_datasets_end(args):
    transform = Prefetch2Device(device=args.device)
    ...
    # Generating Datasets.
    train_dataset = CustomSliceDataEnd(
        root=Path(args.data_root) / f'{args.challenge}_train',
        transform=transform,
        challenge=args.challenge,
        sample_rate=args.sample_rate_train,
        start_slice=args.start_slice_train,
        end_slice=args.end_slice_train,
        use_gt=args.use_gt
    )

    val_dataset = CustomSliceDataEnd(
        root=Path(args.data_root) / f'{args.challenge}_val',
        transform=transform,
        challenge=args.challenge,
        sample_rate=args.sample_rate_val,
        start_slice=args.start_slice_val,
        end_slice=args.end_slice_val,
        use_gt=args.use_gt
    )
    return train_dataset, val_dataset
...
def create_prefetch_data_loaders_end(args):
    train_dataset, val_dataset = create_prefetch_datasets_end(args)
