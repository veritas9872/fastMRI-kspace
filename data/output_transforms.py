...
class OutputReplaceTransformSemiK2(nn.Module):
    """
    Outputs are expected to be k-space data that need reshaping and conversion to the image domain.
    Inputs and targets are expected to be scaled already.
    Currently, the implementation expects only 1 batch.
    ...
    def forward(self, kspace_outputs, targets, extra_params):
        ...
        semi_kspace_recons = (nchw_to_kspace(kspace_outputs) + 0.05)
        # kspace_recons = torch.fft(semi_kspace_recons, signal_ndim=3, normalized=True)
        kspace_recons = fft1(semi_kspace_recons * extra_params['norm_mask'], direction='height')

        assert kspace_recons.shape == targets['kspace_targets'].shape, 'Reconstruction and target sizes are different.'

        cmg_recons = ifft1(semi_kspace_recons * extra_params['norm_mask'], direction='width')

        img_recons = complex_abs(cmg_recons)

        recons = {'kspace_recons': kspace_recons, 'cmg_recons': cmg_recons, 'img_recons': img_recons, 'semi_kspace_recons': semi_kspace_recons}
...
class OutputReplaceTransformPhaseK(nn.Module):
    """
    Outputs are expected to be k-space data that need reshaping and conversion to the image domain.
    Inputs and targets are expected to be scaled already.
    Currently, the implementation expects only 1 batch.
    ...
    def forward(self, kspace_outputs, targets, extra_params):
        ...
        # For removing width dimension padding. Recall that k-space form has 2 as last dim size.
        left = (kspace_outputs.size(-1) - targets['kspace_targets'].size(-1)) // 2
        right = left + targets['kspace_targets'].size(-1)
        ...
        # kspace_recons = torch.stack([extra_params['norm_mask'] * torch.cos(kspace_outputs), extra_params['norm_mask'] * torch.sin(kspace_outputs)], dim=-1)
        kspace_recons = kspace_outputs
        assert kspace_recons.shape == targets['kspace_targets'].shape, 'Reconstruction and target sizes are different.'

        cmg_recons = ifft2(torch.stack([extra_params['norm_mask'] * torch.cos(kspace_recons), extra_params['norm_mask'] * torch.sin(kspace_recons)], dim=-1))
