...
class CycleUnpairedData(Dataset):
    ...
    def __init__(self, root, transform, challenge, sample_rate=1, start_slice=0, use_gt=False, center_crop=True):
        ...
        self.fcf_examples = self.examples
    ...
    def __getitem__(self, idx):
        file_path, slice_num = self.examples[idx]
        fcf_file_path, fcf_slice_num = self.fcf_examples[idx]
        with h5py.File(file_path, mode='r') as data:
            attrs = dict(data.attrs)
            k_slice_cfc = data['kspace'][slice_num]
            k_slice_fcf = data['kspace'][fcf_slice_num]
            if (self.recons_key in data) and self.use_gt:
                target_slice = data[self.recons_key][slice_num]
            else:
                target_slice = None
        # print(k_slice_cfc.shape)
        # print(k_slice_fcf.shape)
        # print(target_slice.shape)
        # print(len(attrs))
        # print(file_path.name)
        # print(slice_num)
        return self.transform(k_slice_cfc, k_slice_fcf, target_slice, attrs, file_path.name, slice_num)
